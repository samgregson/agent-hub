...
The App renders a deliberately simple foundation-bearing-check card: it turns
the Artifact's portable `available`/`unavailable` state into a pass/review
presentation and calls the normal replacement tool when a user changes it.
The displayed loads and utilisation are illustrative, not an engineering
calculation.
