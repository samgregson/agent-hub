...
def create_project_files_router(
    ...
    @router.get("", response_model=ProjectFilePreviewResponse)
    async def preview_file(
        ...
        try:
            await projects.load(ProjectAccess(subject=context.subject), project_id)
        ...
        try:
            file = await project_files.load(project_id, path.removeprefix("/project"))
