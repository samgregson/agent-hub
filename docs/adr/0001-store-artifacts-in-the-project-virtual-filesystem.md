...
Agent Hub is the sole durable owner of Artifact Documents and stores them in the project Virtual Filesystem. Plugins remain stateless with respect to Artifact data: they own payload schemas, validation, transformations, and UI templates. New Plugin integrations persist successful structured MCP Tool Result Snapshots under ADR-0006; the earlier complete-document semantic-operation protocol remains only for existing compatibility paths.
